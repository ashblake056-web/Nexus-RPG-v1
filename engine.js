...
const BUILDINGS_BY_MAP={
  bootville:[{x:7,y:3,key:'lab',w:3,h:3}],
  gridlock: [{x:7,y:2,key:'gym',w:3,h:3},{x:7,y:4,key:'center',w:3,h:3},{x:19,y:4,key:'shop',w:2,h:2}],
  route2:   [],
  ironhaven:[{x:7,y:2,key:'gym',w:3,h:3},{x:7,y:4,key:'center',w:3,h:3},{x:19,y:4,key:'shop',w:2,h:2},{x:16,y:8,key:'ironhaven_factory',w:3,h:3},{x:19,y:14,key:'syntek_plant',w:3,h:3}],
};
...
// ══════════════════════════════════════════════
const MAP_TINT={
  bootville: {grass:'rgba(60,180,30,0.12)',  path:'rgba(200,180,120,0.10)',wall:'rgba(100,80,40,0.15)'},
  gridlock:  {grass:'rgba(0,20,0,0.30)',     path:'rgba(0,80,120,0.18)',  wall:'rgba(10,10,60,0.25)'},
  route2:    {grass:'rgba(60,40,10,0.25)',   path:'rgba(80,60,30,0.20)',  wall:'rgba(60,40,20,0.25)'},
  ironhaven: {grass:'rgba(20,20,20,0.35)',   path:'rgba(50,30,0,0.20)',   wall:'rgba(30,20,10,0.30)'},
};
...
function drawTile(ctx,t,tx,ty,ts){
  ...
  const cityOverrides={
    bootville: { grass:'bootville_grass', tall:'bootville_grass', path:'bootville_path' },
    gridlock:  { grass:'gridlock_grass',  tall:'gridlock_grass',  path:'gridlock_path'  },
    route2:    { grass:'route2_ground',   tall:'route2_tall',     path:'route2_ground'  },
    ironhaven: { grass:'ironhaven_path',  tall:'ironhaven_path',  path:'ironhaven_path' },
  };
  const cityMap=cityOverrides[G.map]||{};
  const baseKey=imgMap[t];
  const cityKey=cityMap[baseKey];
...
function drawMap(){
  ...
  // NPCs
  ctx.textAlign='center';ctx.textBaseline='middle';
  const NPC_SPRITE_MAP={'👩‍🔬':'ada','👩‍⚕️':'nurse','🛒':'shop','🧑':'kael'};
...
function canWalk(x,y){
  ...
  for(const b of getBuildingsForMap(G.map)){
    if(x>=b.x&&x<b.x+b.w&&y>=b.y&&y<b.y+b.h-1)return false;
  }
  return true;
}
function move(dir){
...
function updateBattleHPBars(){
  ...
  document.getElementById('player-battle-name').textContent=battleState.player.name+getStatusTag('player');
  document.getElementById('enemy-name').textContent=battleState.enemy.name+getStatusTag('enemy');
}
...
function executePlayerMove(moveName){
  ...
  if(bs.playerStatus&&bs.playerStatus.type==='conf'&&Math.random()<.33){
    ...
    updateBattleHPBars();
    if(bs.player.hp<=0){setTimeout(()=>handlePlayerFaint(),1200);return;}
...
function _doPlayerAttack(moveName,md){
  ...
  if(eotMsg){setBattleLog(eotMsg);setTimeout(enemyTurn,1400);}
  else setTimeout(enemyTurn,1400);
}
...
function _doEnemyAttack(afterCb){
  ...
  if(bs.enemyStatus&&bs.enemyStatus.type==='conf'&&Math.random()<.33){
    ...
    if(eot)setTimeout(()=>{setBattleLog(eot);setTimeout(()=>{if(afterCb)afterCb();},1200);},1400);
    else setTimeout(()=>{if(afterCb)afterCb();},1400);
    return;
  }
  ...
  if(eot){
    setBattleLog(eot);
    setTimeout(()=>{if(afterCb)afterCb();},1400);
  } else {
    setTimeout(()=>{if(afterCb)afterCb();},1400);
  }
}
...
function handlePlayerFaint(){
  ...
  else{setBattleLog(bs.player.name+' fainted!\nChoose your next Pokémon!');setTimeout(openSwitchMenu,1200);}
}
...
function endBattle(won,caught){
  ...
  if(won&&!caught){
    ...
    while(lead.xp>=(lead.xpNext||100)){
      ...
      if(evo&&lead.level>=evo.level&&!pendingEvolution) pendingEvolution={pokemon:lead,evo};
    }
    if(bs._isKael){
